...
def perturbation_workspace_server(
    ...
    async def _run_analysis(
        ...
        try:
            with perf(
                session.id, "perturbation.workspace", "corr_all_pairs_sql", kind="data"
            ):
    ...
    def pair_box_container() -> ui.Tag:
        ...
        """
        # Unmount the plotly figures when the Perturbation tab is not active so
        # they do not stay resident in the DOM (large figures across all tabs
        # freeze the browser). They re-render when the tab becomes active again.
        if active_tab is not None and active_tab() != "Perturbation":
            return ui.span()
    ...
    def scatter_container() -> ui.Tag:
        ...
        """
        # Unmount the scatter figures when the Perturbation tab is not active so
        # they do not stay resident in the DOM (see pair_box_container).
        if active_tab is not None and active_tab() != "Perturbation":
            return ui.span()
        if _run_analysis.status() != "success":
            return ui.span()
