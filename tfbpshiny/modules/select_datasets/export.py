...
def build_export_tarball(
    ...
    A thread-safe DuckDB cursor is created via ``conn.cursor()`` so
    this function can safely run in a worker thread while the main event
    loop continues to use ``conn`` for reactive queries.

    :param datasets: Dataset export specs (SQL queries, not DataFrames).
    :param conn: Read-only DuckDB connection for executing queries.
    ...
    """
    # Create a thread-local cursor so we don't race with the event loop.
    # DuckDB cursors are safe for concurrent reads.
    cursor = conn.cursor()
