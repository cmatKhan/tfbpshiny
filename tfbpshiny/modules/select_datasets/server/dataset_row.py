...
def dataset_row_server(
    ...
    *,
    ...
    def _sync_toggle_to_dom() -> None:
        """
        Keep the DOM switch in sync with ``toggle_state``.
    ...
    def _open_filter_modal() -> None:
        ...
        # Build union of categorical levels for each common field across all active
        # datasets, so all valid values are selectable in the modal.
        ...
        for cf_field in common_fields:
            ...
            for other_db in all_active:
                ...
                try:
                    ...
                    other_df = conn.execute(other_sql, other_params).df()
        ...
        try:
            ...
            reg_df = conn.execute(reg_sql, reg_params).df()
